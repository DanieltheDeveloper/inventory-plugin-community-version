...
FString UItemEquipmentTypeDataAsset::GetIdentifierString() const
{
	return GetPrimaryAssetId().ToString();
}

FPrimaryAssetId UItemEquipmentTypeDataAsset::GetPrimaryAssetId() const
{
...
FText UItemEquipmentTypeDataAsset::GetName_Implementation()
{
	return Name;
}

#if WITH_EDITORONLY_DATA
void UItemEquipmentTypeDataAsset::PreSave(FObjectPreSaveContext ObjectSaveContext)
{
	...
	Delegate = UPackage::PackageSavedWithContextEvent.AddLambda([this](const FString& InString, const UPackage* InPackage, const FObjectPostSaveContext& InContext)
	{
		...
		{
			UE_LOG(InventorySystem, Warning, TEXT("[UItemEquipmentTypeDataAsset|%s][PreSave]: Saved %s"), *GetFName().ToString(), *InPackage->GetFName().ToString());
...
void UItemEquipmentTypeDataAsset::PostSaveRoot(FObjectPostSaveRootContext ObjectSaveContext)
{
...
void UItemEquipmentTypeDataAsset::RerunAllItemDropConstructionScripts()
{
	const FAssetRegistryModule& AssetRegistryModule = FModuleManager::LoadModuleChecked<FAssetRegistryModule>("AssetRegistry");
	
	if (!AssetRegistryModule.IsValid())
	{
		UE_LOG(InventorySystem, Warning, TEXT("[UItemEquipmentTypeDataAsset|%s][RerunAllItemDropConstructionScripts]: AssetRegistryModule is invalid"), *GetFName().ToString());
		return;
	}

	const IAssetRegistry& AssetRegistry = AssetRegistryModule.Get();

	const FText TaskTitle = FText(NSLOCTEXT("InventorySystem", "PropagateSystemEquipmentTypeDataChange", "Propagate Equipment Type Data Asset Change"));
	...
	Filter.ClassPaths= TArray{FTopLevelAssetPath(UBlueprint::StaticClass()), FTopLevelAssetPath(AActor::StaticClass()), FTopLevelAssetPath(UActorComponent::StaticClass()), FTopLevelAssetPath(AItemDrop::StaticClass()), FTopLevelAssetPath(UItemDataAsset::StaticClass()), FTopLevelAssetPath(UItemEquipmentDataAsset::StaticClass())};
	...
	{
		SlowTask.EnterProgressFrame(1, FText::Format(FText::FromString(TEXT("{0}")), NSLOCTEXT("InventorySystem", "PropagateSystemEquipmentTypeDataChangeDesc", "Propagate Equipment Type Data Asset Change for Assets")));
